...
\begin{tikzpicture}[level distance=3.5cm,sibling distance=.4cm, 
...
\end{tikzpicture}
    ...
    \caption{Esempio di albero degli stati per il gioco del tris, annotato con il valore \emph{minimax} per ogni stato. Il giocatore \textsc{Min} gioca con $\times$, \textsc{Max} gioca con $\bigcirc$.}
    \label{gametree}
\end{figure}
\end{center}
